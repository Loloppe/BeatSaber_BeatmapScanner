                    ...
                    if (SongDetailsUtil.songDetails != null && beatmapKey.levelId.Contains("custom"))
                    {
                        ...
                        if (colorNotes.Count > 0 && beatmapLevel.beatsPerMinute > 0)
                        {
                            if (info.noteJumpMovementSpeed != 0)
                            {
                                ...
                                if (red.Count() > 0)
                                {
                                    ebpm= Helper.GetEBPM(red, beatmapLevel.beatsPerMinute, info.noteJumpMovementSpeed, false);
                                }
                                var blue = colorNotes.Where(c => c.colorType == ColorType.ColorB).ToList();
                                if (blue.Count() > 0)
                                {
                                    ebpm = Math.Max(ebpm, Helper.GetEBPM(blue, beatmapLevel.beatsPerMinute, info.noteJumpMovementSpeed, true));
                                }
                                ...
                                if (result)
                                {
                                    ...
                                    if (folderPath != null)
                                    {
                                        ...
                                        if (singleDiff != null)
                                        {
                                            ...
                                            {
                                                Data[0] = Helper.DetectCrouchWalls(singleDiff.Difficulty.Data.Walls);
                                            }
                                        }
                                    }
                                    if (ratings != null)
                                    {
                                        ...
                                        Data[3] = ratings.FirstOrDefault().Reset;
                                    }
                                }
                            }
